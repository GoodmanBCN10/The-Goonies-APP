...
void runDhtSearch(const uint8_t infoHash[20],
    ...
    if (!engine) {
        printf("[magnet] dht unavailable, resolving without it\n");
        return;
    }
...
bool receiveMetadataPiece(socket_t fd,
    ...
    for (int message = 0; message < 32; ++message) {
        if (!recvFrame(fd, frame)) {
            printf("[magnet] peer frame receive timed out\n");
            return false;
        }
        ...
        if (!be_decode(&cursor, end, &header) || header.type != BE_DICT) {
            printf("[magnet] invalid metadata message header\n");
            return false;
        }
        be_node_t type;
        be_node_t index;
        if (!be_dict_get(header.buf, header.buf + header.raw_len, "msg_type", 8,
            ...
            printf("[magnet] incomplete metadata message header\n");
            return false;
        }
        if (type.ival == 2) {
            printf("[magnet] peer rejected metadata piece %lld\n",
...
bool fetchMetadataFromPeer(const uint8_t* compact,
    ...
    if (!connectPeer(compact, spec.infoHash, peerId, fd)) {
        printf("[magnet] peer %u/%u connect or handshake failed\n",
                peerIndex + 1, peerCount);
        net_close(fd);
        return false;
    }
    printf("[magnet] peer %u/%u BitTorrent handshake ok\n",
            peerIndex + 1, peerCount);
    ...
    if (!negotiateMetadata(fd, extension, metadataSize)) {
        printf("[magnet] peer %u/%u has no usable ut_metadata\n",
                peerIndex + 1, peerCount);
        net_close(fd);
        return false;
    }
    printf("[magnet] peer %u/%u ut_metadata=%u size=%zu\n",
    ...
    while (!cancelled && !stopWorkers && now_ms() < deadline &&
        ...
        if (!receiveMetadataPiece(fd, piece, bytes, byteCount, frame)) {
            printf("[magnet] peer %u/%u metadata receive failed\n",
                    peerIndex + 1, peerCount);
            net_close(fd);
            return false;
        }
        ...
        if (byteCount != expected) {
            printf("[magnet] peer %u/%u wrong metadata piece size %zu/%zu\n",
...
bool MagnetResolver::resolveToFile(const std::string& uri,
    ...
    if (presetInfo && !presetInfo->empty()) {
        ...
        if (buildTorrent(spec, *presetInfo, torrent, presetError)) {
            printf("[magnet] resolved from catalog info_dict (%zu bytes)\n",
                    presetInfo->size());
            return writeTorrentAtomic(path, torrent, error);
        }
        printf("[magnet] catalog info_dict rejected: %s\n",
                presetError.c_str());
    }
    ...
    for (const std::string& tracker : trackers) {
        ...
        if (result.tracker_failure) {
            ...
            printf("[magnet] tracker %s rejected hash: %s\n",
        ...
        if (count)
            printf("[magnet] tracker %s added %u peers, total=%u\n",
                    tracker.c_str(), count,
    ...
    dhtThread.join();
    {
        ...
        if (dhtCount) {
            ...
            printf("[magnet] dht added %u peers, total=%u\n",
    ...
    if (peerCount <= kPexThinThreshold && !cancelled) {
        ...
        if (added)
            printf("[magnet] pex added %u peers, total=%u\n", added,
                    static_cast<unsigned>(peers.size() / 6));
        ...
        workers.emplace_back([&] {
            while (!cancelled && !stopWorkers && now_ms() < deadline) {
                ...
                if (!buildTorrent(spec, candidate, torrentProbe, probeError)) {
                    printf("[magnet] peer %u/%u metadata rejected: %s\n",
