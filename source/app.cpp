...
#include "core_utils/profile.hpp"
#include "core_utils/thread.hpp"
#include "core_utils/devoptab.hpp"
...
void App::Loop() {
    ...
    while (!m_quit && appletMainLoop()) {
        ...
        frontend::gfx::updateHighlightAnimation();
...
auto App::Push(std::unique_ptr<frontend::Widget>&& widget) -> void {
...
void App::Notify(const std::string& text, frontend::NotifEntry::Side side) {
    g_app->m_notif_manager.Push({text, side});
}

void App::Notify(frontend::NotifEntry entry) {
    g_app->m_notif_manager.Push(entry);
}

void App::NotifyPop(frontend::NotifEntry::Side side) {
    g_app->m_notif_manager.Pop(side);
}

void App::NotifyClear(frontend::NotifEntry::Side side) {
...
Result App::PushErrorBox(Result rc, const std::string& message) {
    if (R_FAILED(rc)) {
        App::Push<frontend::ErrorBox>(rc, message);
    }
    return rc;
}
...
void App::SetReplaceHbmenuEnable(bool enable) {
    if (App::GetReplaceHbmenuEnable() != enable) {
        ...
        if (!enable) {
            // check we have already replaced hbmenu with GooniesInstaller
            NacpStruct hbmenu_nacp{};
            if (R_SUCCEEDED(nro_get_nacp("/hbmenu.nro", hbmenu_nacp))) {
                if (std::strcmp(hbmenu_nacp.lang[0].name, "GooniesInstaller")) {
                    return;
                }
            }

            // ask user if they want to restore hbmenu
            App::Push<frontend::OptionBox>(
                ...
                "Back"_i18n, "Restore"_i18n, 1, [hbmenu_nacp](auto op_index){
                    ...
                    NacpStruct GooniesInstaller_nacp;
                    fs::FsPath GooniesInstaller_path = "/switch/GooniesInstaller/GooniesInstaller.nro";
                    Result rc;

                    // first, try and backup GooniesInstaller, its not super important if this fails.
                    rc = nro_get_nacp(GooniesInstaller_path, GooniesInstaller_nacp);
                    if (R_FAILED(rc) || std::strcmp(GooniesInstaller_nacp.lang[0].name, "GooniesInstaller")) {
                        GooniesInstaller_path = "/switch/GooniesInstaller.nro";
                        rc = nro_get_nacp(GooniesInstaller_path, GooniesInstaller_nacp);
                    }

                    if (R_SUCCEEDED(rc) && !std::strcmp(GooniesInstaller_nacp.lang[0].name, "GooniesInstaller")) {
                        if (IsVersionNewer(GooniesInstaller_nacp.display_version, hbmenu_nacp.display_version)) {
                            if (R_FAILED(rc = g_app->m_fs->copy_entire_file(GooniesInstaller_path, "/hbmenu.nro"))) {
                                log_write("failed to copy entire file: %s 0x%X module: %u desc: %u\n", GooniesInstaller_path.s, rc, R_MODULE(rc), R_DESCRIPTION(rc));
                            } else {
                                log_write("success with updating hbmenu!\n");
                            }
                        }
                    } else {
                        // GooniesInstaller doesn't yet exist, create a new file.
                        GooniesInstaller_path = "/switch/GooniesInstaller/GooniesInstaller.nro";
                        g_app->m_fs->CreateDirectoryRecursively("/switch/GooniesInstaller/");
                        g_app->m_fs->copy_entire_file(GooniesInstaller_path, "/hbmenu.nro");
                    }
                    ...
                    if (R_FAILED(rc = g_app->m_fs->copy_entire_file("/hbmenu.nro", "/switch/hbmenu.nro")))  {
                        // try and restore GooniesInstaller in a last ditch effort.
                        if (R_FAILED(rc = g_app->m_fs->copy_entire_file("/hbmenu.nro", GooniesInstaller_path))) {
                            ...
                            App::Push<frontend::OptionBox>(
                                "Failed to restore hbmenu, using GooniesInstaller instead"_i18n,
                                "OK"_i18n
                            );
                        }
                        return;
                    ...
                    if (IsHbmenu()) {
                        App::Push<frontend::OptionBox>(
                            "Restored hbmenu, closing GooniesInstaller"_i18n,
                            "OK"_i18n, [](auto) {
                                App::Exit();
                            }
                        );
...
auto App::Install(OwoConfig& config) -> Result {
    App::Push<frontend::ProgressBox>(0, "Installing Forwarder"_i18n, config.name, [config](auto pbox) mutable -> Result {
...
auto App::Install(frontend::ProgressBox* pbox, OwoConfig& config) -> Result {
    ...
    if (config.logo.empty()) {
        g_app->m_fs->read_entire_file("/config/GooniesInstaller/logo/NintendoLogo.png", config.logo);
    }

    if (config.gif.empty()) {
        g_app->m_fs->read_entire_file("/config/GooniesInstaller/logo/StartupMovie.gif", config.gif);
    }
...
void DrawElement(const Vec4& v, ThemeEntryID id) {
    ...
    switch (e.type) {
        ...
        case ElementType::Texture: {
            ...
            frontend::gfx::drawRect(g_app->vg, v, paint);
        } break;
        case ElementType::Colour: {
            frontend::gfx::drawRect(g_app->vg, v, e.colour);
        } break;
    }
}
...
void App::ScanThemeEntries() {
    ...
    ScanThemes("/config/GooniesInstaller/themes/");
}
...
Result App::SetDefaultBackgroundMusic(fs::Fs* fs, const fs::FsPath& path) {
    constexpr const char* base_path = "/config/GooniesInstaller/themes/default_music.";
...
App::App(const char* argv0) {
    ...
    // currrent load time is 60ms without logs, 90 with (down from 230ms).
        {
            ...
            m_fs->CreateDirectoryRecursively("/config/GooniesInstaller");
            m_fs->CreateDirectory("/config/GooniesInstaller/assoc");
            m_fs->CreateDirectory("/config/GooniesInstaller/themes");
            m_fs->CreateDirectory("/config/GooniesInstaller/i18n");
            m_fs->CreateDirectory("/config/GooniesInstaller/mount");
        }
    ...
    if (App::GetLogEnable()) {
        App::Notify("Warning! Logs are enabled, GooniesInstaller will run slowly!"_i18n);
    }
    ...
    // not sure if these are meant to be deleted or not...
    {
        ...
        if (font_standard.address && font_extended.address) {
            ...
            for (auto type : lang_font) {
                if (R_SUCCEEDED(plGetSharedFontByType(&font_lang, type)) && font_lang.address) {
                    char name[32];
                    std::snprintf(name, sizeof(name), "Lang_%u", font_lang.type);
                    auto l_font = nvgCreateFontMem(this->vg, name, (unsigned char*)font_lang.address, font_lang.size, 0);
                    nvgAddFallbackFontId(this->vg, standard_font, l_font);
                } else {
                    log_write("failed plGetSharedFontByType(%d)\n", type);
                }
            }
        } else {
            log_write("Standard fonts failed to load!\n");
        }
    }
...
void App::DisplayThemeOptions(bool left_side) {
    frontend::SidebarEntryArray::Items theme_items{};
    const auto theme_meta = App::GetThemeMetaList();
    for (auto& p : theme_meta) {
        theme_items.emplace_back(p.name);
    }

    auto options = std::make_unique<frontend::Sidebar>("Theme Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));

    options->Add<frontend::SidebarEntryArray>("Select Theme"_i18n, theme_items, [](s64& index_out){
        App::SetTheme(index_out);
    }, App::GetThemeIndex(), "Customise the look of GooniesInstaller by changing the theme"_i18n);

    options->Add<frontend::SidebarEntryBool>("Music"_i18n, App::GetThemeMusicEnable(), [](bool& enable){
    ...
    options->Add<frontend::SidebarEntryBool>("Show IP address"_i18n, App::GetApp()->m_show_ip_addr,
    ...
    auto remove_music = options->Add<frontend::SidebarEntryCallback>("Remove Background Music"_i18n, [](){
...
void App::DisplayMenuOptions(bool left_side) {
    auto options = std::make_unique<frontend::Sidebar>("Menus"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));

    for (auto& e : frontend::menu::main::GetMenuMenuEntries()) {
        ...
        auto entry = options->Add<frontend::SidebarEntryCallback>(i18n::get(e.title), [e](){
            App::Push(e.func(frontend::menu::MenuFlag_None));
        }, i18n::get(e.info));
    ...
    if (App::IsApplication()) {
        options->Add<frontend::SidebarEntryCallback>("Web"_i18n, [](){
            // add some default entries, will use a config file soon so users can set their own.
            frontend::PopupList::Items items;
            ...
            items.emplace_back("https://github.com/ITotalJustice/GooniesInstaller/wiki");
            items.emplace_back("Enter custom URL"_i18n);

            App::Push<frontend::PopupList>(
...
void App::DisplayAdvancedOptions(bool left_side) {
    auto options = std::make_unique<frontend::Sidebar>("Advanced Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));

    frontend::SidebarEntryArray::Items text_scroll_speed_items;
    ...
    frontend::SidebarEntryArray::Items menu_items;
    for (auto& e : frontend::menu::main::GetMenuMenuEntries()) {
        if (!e.IsShortcut()) {
            continue;
        }
    ...
    options->Add<frontend::SidebarEntryBool>("Logging"_i18n, App::GetLogEnable(), [](bool& enable){
        App::SetLogEnable(enable);
    }, "Logs to /config/GooniesInstaller/log.txt"_i18n);

    options->Add<frontend::SidebarEntryBool>("Replace hbmenu on exit"_i18n, App::GetReplaceHbmenuEnable(), [](bool& enable){
        App::SetReplaceHbmenuEnable(enable);
    },  i18n::get("hbmenu_replace_info",
            "When enabled, it replaces /hbmenu.nro with GooniesInstaller, creating a backup of hbmenu to /switch/hbmenu.nro\n\n"
            "Disabling will give you the option to restore hbmenu."));

    options->Add<frontend::SidebarEntryCallback>("Add / modify mounts"_i18n, [](){
        devoptab::DisplayDevoptabSideBar();
            ...
            "Any changes made will require restarting GooniesInstaller to take effect."));

    options->Add<frontend::SidebarEntryBool>("Boost CPU during transfer"_i18n, App::GetApp()->m_progress_boost_mode,
    ...
    options->Add<frontend::SidebarEntryArray>("Text scroll speed"_i18n, text_scroll_speed_items, [](s64& index_out){
        App::SetTextScrollSpeed(index_out);
    }, App::GetTextScrollSpeed(), "Change how fast the scrolling text updates"_i18n);

    options->Add<frontend::SidebarEntryArray>("Set center menu"_i18n, menu_items, [menu_names](s64& index_out){
        ...
        if (g_app->m_center_menu.Get() != e) {
            ...
            App::Push<frontend::OptionBox>(
                "Press OK to restart GooniesInstaller"_i18n, "OK"_i18n, [](auto){
                    App::ExitRestart();
                }
            );
        }
    ...
    options->Add<frontend::SidebarEntryArray>("Set left-side menu"_i18n, menu_items, [menu_names](s64& index_out){
        const auto e = menu_names[index_out];
        if (g_app->m_left_menu.Get() != e) {
            // swap menus around.
            if (g_app->m_center_menu.Get() == e) {
    ...
    options->Add<frontend::SidebarEntryArray>("Set right-side menu"_i18n, menu_items, [menu_names](s64& index_out){
        const auto e = menu_names[index_out];
    ...
    options->Add<frontend::SidebarEntryCallback>("Install options"_i18n, [left_side](){
    ...
    options->Add<frontend::SidebarEntryCallback>("Export options"_i18n, [left_side](){
        App::DisplayDumpOptions(left_side);
    },  "Change the export options."_i18n);
}

void App::DisplayInstallOptions(bool left_side) {
    auto options = std::make_unique<frontend::Sidebar>("Install Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));

    frontend::SidebarEntryArray::Items install_items;
    install_items.push_back("System memory"_i18n);
    install_items.push_back("microSD card"_i18n);

    options->Add<frontend::SidebarEntryBool>("Enable sysmmc"_i18n, App::GetInstallSysmmcEnable(), [](bool& enable){
        ShowEnableInstallPromptOption(g_app->m_install_sysmmc, enable);
    }, "Enables installing whilst in sysMMC mode."_i18n);

    options->Add<frontend::SidebarEntryBool>("Enable emummc"_i18n, App::GetInstallEmummcEnable(), [](bool& enable){
        ShowEnableInstallPromptOption(g_app->m_install_emummc, enable);
    }, "Enables installing whilst in emuMMC mode."_i18n);

    options->Add<frontend::SidebarEntryArray>("Install location"_i18n, install_items, [](s64& index_out){
        App::SetInstallSdEnable(index_out);
    }, (s64)App::GetInstallSdEnable());

    options->Add<frontend::SidebarEntryBool>("Allow downgrade"_i18n, App::GetApp()->m_allow_downgrade,
        "Allows for installing title updates that are lower than the currently installed update."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip if already installed"_i18n, App::GetApp()->m_skip_if_already_installed,
        "Skips installing titles / ncas if they're already installed."_i18n);

    options->Add<frontend::SidebarEntryBool>("Ticket only"_i18n, App::GetApp()->m_ticket_only,
        "Installs tickets only, useful if the title was already installed however the tickets were missing or corrupted."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip base"_i18n, App::GetApp()->m_skip_base,
        "Skips installing the base application."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip patch"_i18n, App::GetApp()->m_skip_patch,
        "Skips installing updates."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip dlc"_i18n, App::GetApp()->m_skip_addon,
        "Skips installing DLC."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip data patch"_i18n, App::GetApp()->m_skip_data_patch,
        "Skips installing DLC update (data patch)."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip ticket"_i18n, App::GetApp()->m_skip_ticket,
        "Skips installing tickets, not recommended."_i18n);

    options->Add<frontend::SidebarEntryBool>("Skip NCA hash verify"_i18n, App::GetApp()->m_skip_nca_hash_verify,
    ...
    options->Add<frontend::SidebarEntryBool>("Skip RSA header verify"_i18n, App::GetApp()->m_skip_rsa_header_fixed_key_verify,
    ...
    options->Add<frontend::SidebarEntryBool>("Skip RSA NPDM verify"_i18n, App::GetApp()->m_skip_rsa_npdm_fixed_key_verify,
    ...
    options->Add<frontend::SidebarEntryBool>("Ignore distribution bit"_i18n, App::GetApp()->m_ignore_distribution_bit,
    ...
    options->Add<frontend::SidebarEntryBool>("Convert to common ticket"_i18n, App::GetApp()->m_convert_to_common_ticket,
    ...
    options->Add<frontend::SidebarEntryBool>("Convert to standard crypto"_i18n, App::GetApp()->m_convert_to_standard_crypto,
        i18n::get("titlekey_crypto_info",
            "[Requires keys] Converts titlekey to standard crypto, also known as \"ticketless\".\n\n"
            "It is recommended to keep this disabled."));

    options->Add<frontend::SidebarEntryBool>("Lower master key"_i18n, App::GetApp()->m_lower_master_key,
    ...
    options->Add<frontend::SidebarEntryBool>("Lower system version"_i18n, App::GetApp()->m_lower_system_version,
...
void App::DisplayDumpOptions(bool left_side) {
    auto options = std::make_unique<frontend::Sidebar>("Export Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));

    frontend::SidebarEntryArray::Items nsz_level_items;
    for (auto& e : NSZ_COMPRESS_LEVEL_OPTIONS) {
        nsz_level_items.emplace_back(i18n::get(e.name));
    }

    frontend::SidebarEntryArray::Items nsz_thread_items;
    for (auto& e : NSZ_COMPRESS_THREAD_OPTIONS) {
        nsz_thread_items.emplace_back(i18n::get(e.name));
    }

    frontend::SidebarEntryArray::Items nsz_block_items;
    for (auto& e : NSZ_COMPRESS_BLOCK_OPTIONS) {
        nsz_block_items.emplace_back(i18n::get(e.name));
    }

    options->Add<frontend::SidebarEntryBool>(
    ...
    options->Add<frontend::SidebarEntryArray>("NSZ level"_i18n, nsz_level_items, [](s64& index_out){
    ...
    options->Add<frontend::SidebarEntryArray>("NSZ threads"_i18n, nsz_thread_items, [](s64& index_out){
    ...
    options->Add<frontend::SidebarEntryBool>(
        ...
        i18n::get("block_compress_info",
            ...
            "NOTE: GooniesInstaller does not yet support mounting NCZ as a file system, but it will be added evntually."
        )
    );

    auto block_size_option = options->Add<frontend::SidebarEntryArray>("NSZ block size"_i18n, nsz_block_items, [](s64& index_out){
...
void App::DisplayFtpOptions(bool left_side) {
    ...
    auto options = std::make_unique<frontend::Sidebar>("FTP Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));
    ...
    options->Add<frontend::SidebarEntryBool>("Enable"_i18n, App::GetFtpEnable(), [](bool& enable){
        App::SetFtpEnable(enable);
    },  "Enable FTP server to run in the background."_i18n);

    options->Add<frontend::SidebarEntryTextInput>(
    ...
    options->Add<frontend::SidebarEntryBool>(
        ...
        i18n::get("mounts_ftp_info",
            ...
            "This folder is contains all of the mounts added to GooniesInstaller, allowing you to acces them over FTP!\n"
...
void App::DisplayMtpOptions(bool left_side) {
    // todo: prompt on exit to restart ftp server if options were changed.
    auto options = std::make_unique<frontend::Sidebar>("MTP Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));
    ...
    options->Add<frontend::SidebarEntryBool>("Enable"_i18n, App::GetMtpEnable(), [](bool& enable){
    ...
    options->Add<frontend::SidebarEntryBool>(
        ...
        i18n::get("mounts_mtp_info",
            "Shows the mounts folder.\n\n"
            "This folder is contains all of the mounts added to GooniesInstaller, allowing you to acces them over MTP!\n"
...
void App::DisplayHddOptions(bool left_side) {
    auto options = std::make_unique<frontend::Sidebar>("HDD Options"_i18n, left_side ? frontend::Sidebar::Side::LEFT : frontend::Sidebar::Side::RIGHT);
    ON_SCOPE_EXIT(App::Push(std::move(options)));

    options->Add<frontend::SidebarEntryBool>("Enable"_i18n, App::GetHddEnable(), [](bool& enable){
    ...
    options->Add<frontend::SidebarEntryBool>("HDD write protect"_i18n, App::GetWriteProtect(), [](bool& enable){
...
App::~App() {
    ...
    log_write("starting to exit\n");
    {
        ...
        // backup hbmenu if it is not GooniesInstaller
        {
            ...
            if (App::GetReplaceHbmenuEnable() && !IsHbmenu()) {
                ...
                if (R_SUCCEEDED(rc = nro_get_nacp("/hbmenu.nro", hbmenu_nacp)) && std::strcmp(hbmenu_nacp.lang[0].name, "GooniesInstaller")) {
                ...
                // ensure that are still GooniesInstaller
                if (R_SUCCEEDED(rc = nro_get_nacp("/hbmenu.nro", hbmenu_nacp)) && !std::strncmp(hbmenu_nacp.lang[0].name, "GooniesInstaller", 512)) {
                    NacpStruct GooniesInstaller_nacp;
                    fs::FsPath GooniesInstaller_path = "/switch/GooniesInstaller/GooniesInstaller.nro";

                    rc = nro_get_nacp(GooniesInstaller_path, GooniesInstaller_nacp);
                    if (R_FAILED(rc) || std::strncmp(GooniesInstaller_nacp.lang[0].name, "GooniesInstaller", 512)) {
                        GooniesInstaller_path = "/switch/GooniesInstaller.nro";
                        rc = nro_get_nacp(GooniesInstaller_path, GooniesInstaller_nacp);
                    }

                    // found GooniesInstaller, now lets get compare version
                    if (R_SUCCEEDED(rc) && !std::strncmp(GooniesInstaller_nacp.lang[0].name, "GooniesInstaller", 512)) {
                        if (IsVersionNewer(hbmenu_nacp.display_version, GooniesInstaller_nacp.display_version)) {
                            if (R_FAILED(rc = m_fs->copy_entire_file(GetExePath(), GooniesInstaller_path))) {
                                log_write("failed to copy entire file: %s 0x%X module: %u desc: %u\n", GooniesInstaller_path.s, rc, R_MODULE(rc), R_DESCRIPTION(rc));
                            } else {
                                log_write("success with updating hbmenu!\n");
                            }
