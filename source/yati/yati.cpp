...
struct ThreadData {
    ...
    void SetReadResult(Result result) {
        ...
        // wake up other threads as they may be waiting on data that never comes.
    ...
    Result SetDecompressBuf(std::vector<u8>& buf, s64 off, s64 size) {
        ...
        while (!read_buffers.ringbuf_free()) {
            if (!decompress_running || !write_running) {
                return 0; // R_SUCCEED();
            }
...
Result Yati::decompressFuncInternal(ThreadData* t) {
    ...
    const auto ncz_flush = [&](s64 size) -> Result {
        ...
        s64 flush_start_off = 0;
        s64 stream_offset = t->decompress_offset - size;
        ...
        for (s64 off = 0; off < size;) {
            if (!ncz_section || !ncz_section->InRange(written)) {
                ...
                if (it == t->ncz_sections.cend()) {
                    auto next_it = std::ranges::find_if(t->ncz_sections, [written](auto& e){
                        return e.offset > written;
                    });
                    
                    s64 gap_size = (next_it != t->ncz_sections.cend()) ? (next_it->offset - written) : (t->write_size.load() - written);
                    
                    if (gap_size > 0) {
                        log_write("[NCZ] skipping gap of size %zu at offset %zu\n", gap_size, written);
                        
                        if (off > flush_start_off) {
                            s64 flush_size = off - flush_start_off;
                            std::vector<u8> flush_buf(inflate_buf.begin() + flush_start_off, inflate_buf.begin() + off);
                            R_TRY(t->SetWriteBuf(flush_buf, flush_size, config.skip_nca_hash_verify, stream_offset));
                            stream_offset += flush_size;
                        }
                        
                        stream_offset += gap_size;
                        
                        written += gap_size;
                        t->decompress_offset += gap_size;
                        flush_start_off = off; 
                        continue;
                    } else {
                        break;
        ...
        if (size > flush_start_off) {
            s64 flush_size = size - flush_start_off;
            std::vector<u8> flush_buf(inflate_buf.begin() + flush_start_off, inflate_buf.begin() + size);
            R_TRY(t->SetWriteBuf(flush_buf, flush_size, config.skip_nca_hash_verify, stream_offset));
            stream_offset += flush_size;
        }
            ...
            while (buf_off < buf.size()) {
                ...
                if (t->ncz_blocks.size()) {
                    if (!ncz_block || !ncz_block->InRange(decompress_buf_off)) {
                        ...
                        if (it == t->ncz_blocks.cend()) {
                            log_write("[NCZ] Reached end of blocks, ignoring padding at offset: %zu\n", decompress_buf_off);
                            break;
                        }
                ...
                if (compressed) {
                    ...
                    while (input.pos < input.size) {
                        ...
                        const auto old_in_pos = input.pos;
                        ...
                        if (output.pos == 0 && input.pos == old_in_pos) {
                            break;
                        }
                        ...
                        if (is_ncz && t->decompress_offset >= t->write_size.load()) {
                            log_write("[NCZ] aborting decompression as decompress_offset (%zu) >= write_size (%zu)\n", t->decompress_offset.load(), t->write_size.load());
                            break;
                        }
                    }
                } else {
...
Result Yati::writeFuncInternal(ThreadData* t) {
    ...
    while (t->write_offset < t->write_size && R_SUCCEEDED(t->GetResults())) {
        s64 stream_off;
        R_TRY(t->GetWriteBuf(buf, stream_off));
        if (buf.empty()) {
            break;
        }

        s64 off{};
        while (off < buf.size() && R_SUCCEEDED(t->GetResults())) {
            const auto wsize = std::min<s64>(t->read_buffer_size, buf.size() - off);
            R_TRY(ncmContentStorageWritePlaceHolder(std::addressof(cs), std::addressof(t->nca->placeholder_id), stream_off + off, buf.data() + off, wsize));

            off += wsize;
            t->write_offset = stream_off + off;
...
Result Yati::Setup(const ConfigOverride& override) {
    ...
    for (size_t i = 0; i < std::size(NCM_STORAGE_IDS); i++) {
        ...
        if (m_out_freed_bytes) {
            s64 initial_free_space = 0;
            s64 final_free_space = 0;
            ncmContentStorageGetFreeSpaceSize(std::addressof(ncm_cs[i]), &initial_free_space);
            ncmContentStorageCleanupAllPlaceHolder(std::addressof(ncm_cs[i]));
            ncmContentStorageGetFreeSpaceSize(std::addressof(ncm_cs[i]), &final_free_space);
            if (final_free_space > initial_free_space) {
                *m_out_freed_bytes += (final_free_space - initial_free_space);
            }
        } else {
            ncmContentStorageCleanupAllPlaceHolder(std::addressof(ncm_cs[i]));
        }
...
Result Yati::InstallNcaInternal(std::span<TikCollection> tickets, NcaCollection& nca) {
    ...
    // For stream installs, we cannot skip existing files. We must overwrite them to consume the stream.
    // If we don't delete the existing content first, CreatePlaceHolder will throw 2002-4057 (0x95DFB202).
    if (!config.skip_if_already_installed && !config.ticket_only) {
        bool exists = false;
        if (R_SUCCEEDED(ncmContentStorageHas(std::addressof(cs), &exists, std::addressof(nca.content_id))) && exists) {
            log_write("Content already exists, deleting for overwrite...\n");
            ncmContentStorageDelete(std::addressof(cs), std::addressof(nca.content_id));
        }
    }
...
Result Yati::InstallCnmtNca(std::span<TikCollection> tickets, CnmtCollection& cnmt, const container::Collections& collections) {
    ...
    if (!config.lower_system_version) {
        if (header.required_sys_version > hosversionGet()) {
            return Result_YatiFirmwareTooLow;
        }
    }
    ...
    if (config.lower_system_version) {
        ...
        auto extended_header = (ncm::ExtendedHeader*)cnmt.extended_header.data();
        if (cnmt.key.type == NcmContentMetaType_Application) {
            req_sys_version = extended_header->application.required_system_version;
        } else if (cnmt.key.type == NcmContentMetaType_Patch) {
            req_sys_version = extended_header->patch.required_system_version;
        }
        if (req_sys_version > 0) {
            u8 major = (req_sys_version >> 26) & 0x3F;
            u8 minor = (req_sys_version >> 20) & 0x3F;
            u8 micro = (req_sys_version >> 16) & 0xF;
            u32 req_hos = MAKEHOSVERSION(major, minor, micro);
            if (hosversionGet() < req_hos) {
                return MAKERESULT(345, 2); // LibnxError_IncompatSysVer
            }
        }
    }
    ...
    for (size_t i = 0; i < std::size(NCM_STORAGE_IDS); i++) {
        ...
        for (const auto& key : keys) {
            ...
            for (const auto& info : infos) {
                ...
                if (m_out_freed_bytes) {
                    s64 file_size = 0;
                    if (R_SUCCEEDED(ncmContentStorageGetSizeFromContentId(std::addressof(cs), &file_size, std::addressof(info.content_id)))) {
                        *m_out_freed_bytes += file_size;
                    }
                }
...
Result InstallInternalStream(source::Base* source, container::Collections collections, const ConfigOverride& override, u64* out_freed_bytes) {
    auto yati = std::make_unique<Yati>(source);
    R_TRY(yati->Setup(override));
    
    // Store pointer to write freed bytes later
    yati->m_out_freed_bytes = out_freed_bytes;
...
Result InstallFromFile(fs::Fs* fs, const fs::FsPath& path, const ConfigOverride& override) {
    ...
    return InstallFromSource(source.get(), path, override, nullptr);
}

Result InstallFromSource(source::Base* source, const fs::FsPath& path, const ConfigOverride& override, u64* out_freed_bytes) {
    ...
    return InstallFromContainer(container.get(), override, out_freed_bytes);
}

Result InstallFromContainer(container::Base* container, const ConfigOverride& override, u64* out_freed_bytes) {
    container::Collections collections;
    R_TRY(container->GetCollections(collections));
    return InstallFromCollections(container->GetSource(), collections, override, out_freed_bytes);
}

Result InstallFromCollections(source::Base* source, const container::Collections& collections, const ConfigOverride& override, u64* out_freed_bytes) {
    if (source->IsStream()) {
        return InstallInternalStream(source, collections, override, out_freed_bytes);
    }
