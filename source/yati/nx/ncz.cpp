...
, m_source{source} {
    ...
    // this isn't needed in GooniesInstaller as i am usually reading in 4mb chunks.
    ...
    *bytes_read_out = 0;
    ...
    while (size) {
        ...
        if (!lru_data) {
            ...
            lru_data->offset = block_id * m_block_size;
        ...
        auto rsize = std::min<s64>(size, (s64)lru_data->data.size() - buf_off);
        if (rsize <= 0) {
            // Trying to read past the decompressed size (truncated padding)
            // Just fill with zeroes or return what we read so far
