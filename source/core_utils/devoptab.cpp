...
#define MOUNT_PATH "/config/GooniesInstaller/mount/"

using namespace GooniesInstaller::frontend;
...
void DevoptabForm::SetupButtons(bool type_change) {
    ...
    const auto callback = this->Add<SidebarEntryCallback>("Save"_i18n, [this](){
        ...
        App::Notify("Mount entry saved. Restart GooniesInstaller to apply changes."_i18n);

        this->SetPop();
    },  "Saves the mount entry.\n\n"
        "NOTE: You must restart GooniesInstaller for changes to take effect!"_i18n);
