...
namespace GooniesInstaller::devoptab::common {
namespace {
...
Result MountNetworkDevice(const CreateDeviceCallback& create_device, size_t file_size, size_t dir_size, const char* name, bool force_read_only) {
    ...
    std::snprintf(config_path, sizeof(config_path), "/config/GooniesInstaller/mount/%s.ini", name);
...
Result PushPullThreadData::CreateAndStart() {
    ...
    R_TRY(core_utils::CreateThread(&thread, thread_func, this));
...
} // GooniesInstaller::devoptab::common

namespace GooniesInstaller::devoptab {

using namespace GooniesInstaller::devoptab::common;
