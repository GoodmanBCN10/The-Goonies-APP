...
namespace GooniesInstaller::thread {
namespace {
...
struct ThreadData {
    ThreadData(frontend::ProgressBox* _pbox, s64 size, const ReadCallback& _rfunc, const DecompressCallback& _dfunc, const WriteCallback& _wfunc, u64 buffer_size);
...
private:
    ...
    frontend::ProgressBox* const pbox;
...
ThreadData::ThreadData(frontend::ProgressBox* _pbox, s64 size, const ReadCallback& _rfunc, const DecompressCallback& _dfunc, const WriteCallback& _wfunc, u64 buffer_size)
: pbox{_pbox}
, rfunc{_rfunc}
, dfunc{_dfunc}
, wfunc{_wfunc}
...
Result TransferInternal(frontend::ProgressBox* pbox, s64 size, const ReadCallback& rfunc, const DecompressCallback& dfunc, const WriteCallback& wfunc, const StartCallback2& sfunc, Mode mode, u64 buffer_size = NORMAL_BUFFER_SIZE) {
    ...
    if (mode == Mode::SingleThreaded) {
        ...
        Thread t_read{};
        R_TRY(core_utils::CreateThread(&t_read, readFunc, std::addressof(t_data)));
        ON_SCOPE_EXIT(threadClose(&t_read));

        Thread t_decompress{};
        R_TRY(core_utils::CreateThread(&t_decompress, decompressFunc, std::addressof(t_data)));
        ON_SCOPE_EXIT(threadClose(&t_decompress));

        Thread t_write{};
        R_TRY(core_utils::CreateThread(&t_write, writeFunc, std::addressof(t_data)));
...
Result Transfer(frontend::ProgressBox* pbox, s64 size, const ReadCallback& rfunc, const WriteCallback& wfunc, Mode mode) {
    return TransferInternal(pbox, size, rfunc, nullptr, wfunc, nullptr, mode);
}

Result Transfer(frontend::ProgressBox* pbox, s64 size, const ReadCallback& rfunc, const DecompressCallback& dfunc, const WriteCallback& wfunc, Mode mode) {
    return TransferInternal(pbox, size, rfunc, dfunc, wfunc, nullptr, mode);
}

Result TransferPull(frontend::ProgressBox* pbox, s64 size, const ReadCallback& rfunc, const StartCallback& sfunc, Mode mode) {
...
Result TransferPull(frontend::ProgressBox* pbox, s64 size, const ReadCallback& rfunc, const StartCallback2& sfunc, Mode mode) {
    return TransferInternal(pbox, size, rfunc, nullptr, nullptr, sfunc, mode);
}

Result TransferUnzip(frontend::ProgressBox* pbox, void* zfile, fs::Fs* fs, const fs::FsPath& path, s64 size, u32 crc32, Mode mode) {
    Result rc;
...
Result TransferZip(frontend::ProgressBox* pbox, void* zfile, fs::Fs* fs, const fs::FsPath& path, u32* crc32, Mode mode) {
    fs::File f;
...
Result TransferUnzipAll(frontend::ProgressBox* pbox, void* zfile, fs::Fs* fs, const fs::FsPath& base_path, const UnzipAllFilter& filter, Mode mode) {
    ...
    for (s64 i = 0; i < ginfo.number_entry; i++) {
        ...
        // this function is used for the updater to change GooniesInstaller.nro to exe path.
...
Result TransferUnzipAll(frontend::ProgressBox* pbox, const fs::FsPath& zip_out, fs::Fs* fs, const fs::FsPath& base_path, const UnzipAllFilter& filter, Mode mode) {
