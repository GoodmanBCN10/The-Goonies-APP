...
namespace goonies {
        ...
        InstallMenuView() : brls::Box(brls::Axis::COLUMN) {
            ...
            buttonsBox->addView(createMenuButton(t("Instalar juegos desde pendrive o disco externo", "Install games from USB drive", "Instale jogos do pendrive ou disco externo"), [this]() {
                ...
                if (count > 0) {
                    ...
                    brls::Application::notify(t("No hay pendrive conectado.", "No USB drive connected.", "NÃ£o hÃ¡ pendrive conectado."));
                }
            }));
    ...
    public:
        ManageMenuView(pipensx::InstalledTitleService* installed, pipensx::DownloadManager* manager, pipensx::GameMetadataService* metadata)
            ...
            brls::Label* titleLabel = new brls::Label();
            titleLabel->setText(t("GestiÃ³n de Consola", "Console Management", "Gerenciamento de console"));
            titleLabel->setFontSize(48);
            titleLabel->setMarginBottom(48);
            centerBox->addView(titleLabel);
    ...
    public:
        LinkGooniesMenuView() : brls::Box(brls::Axis::COLUMN) {
            ...
            buttonsBox->addView(createMenuButton(t("Vincular cuentas offline", "Link accounts offline", "Vincular contas off-line"), []() {
                auto* dialog = new brls::Dialog(t("Â¿Vincular todas las cuentas offline?\nEsto crearÃ¡ una vinculaciÃ³n falsa. Si tenÃ­as una cuenta NNID vinculada, se sobrescribirÃ¡.", "Link all accounts offline?\nThis will create a dummy link. If you had a linked NNID account, it will be overwritten.", "Vincular todas as contas off-line?\nIsso criarÃ¡ um link falso. Se vocÃª tivesse uma conta NNID vinculada, ela serÃ¡ substituÃ­da."));
                dialog->addButton(t("Vincular", "Link", "Link"), [] {
                    ...
                    if (err.empty()) {
                        auto* rebootDialog = new brls::Dialog(t("Â¡Cuentas vinculadas con Ã©xito! Para aplicar los cambios y evitar que la consola crasheÃ©, es obligatorio reiniciar.", "Accounts linked successfully! To apply changes and prevent a system crash, you must reboot.", "Contas vinculadas com sucesso! Para aplicar as alteraÃ§Ãµes e evitar que o console trave, Ã© necessÃ¡ria uma reinicializaÃ§Ã£o."));
                        rebootDialog->addButton(t("Reiniciar", "Reboot", "ReinÃ­cio"), [] {
                        ...
                        auto* errorDialog = new brls::Dialog(t(
                            ("Error al vincular: " + err + "\n\nNOTA: El acceso a los perfiles requiere permisos del sistema. Debes iniciar esta aplicaciÃ³n desde el Ãlbum (Modo Applet) sin pulsar R para poder vincular.").c_str(),
            ...
            buttonsBox->addView(createMenuButton(t("Desvincular cuentas", "Unlink accounts", "Desvincular contas"), []() {
                auto* dialog = new brls::Dialog(t("Â¿Desvincular todas las cuentas?\nSe eliminarÃ¡ la vinculaciÃ³n local y online de tus perfiles.", "Unlink all accounts?\nThis will remove local and online links from your profiles.", "Desvincular todas as contas?\nA vinculaÃ§Ã£o local e on-line dos seus perfis serÃ¡ removida."));
                dialog->addButton(t("Desvincular", "Unlink", "Desvincular"), [] {
                    ...
                    if (err.empty()) {
                        auto* rebootDialog = new brls::Dialog(t("Â¡Cuentas desvinculadas con Ã©xito! Para aplicar los cambios y evitar que la consola crasheÃ©, es obligatorio reiniciar.", "Accounts unlinked successfully! To apply changes and prevent a system crash, you must reboot.", "Contas desvinculadas com sucesso! Para aplicar as alteraÃ§Ãµes e evitar que o console trave, Ã© necessÃ¡ria uma reinicializaÃ§Ã£o."));
                        rebootDialog->addButton(t("Reiniciar", "Reboot", "ReinÃ­cio"), [] {
                            pipensx::linkuser::g_shouldReboot = true;
                            brls::Application::quit();
                        });
                        ...
                        auto* errorDialog = new brls::Dialog(t(
                            ("Error al desvincular: " + err + "\n\nNOTA: El acceso a los perfiles requiere permisos del sistema. Debes iniciar esta aplicaciÃ³n desde el Ãlbum (Modo Applet) sin pulsar R para poder desvincular.").c_str(),
            ...
            // 3. Restaurar copia
            buttonsBox->addView(createMenuButton(t("Restaurar copia de seguridad", "Restore backup", "Restaurar cÃ³pia de seguranÃ§a"), []() {
                ...
                auto* dialog = new brls::Dialog(t(
                    ("Â¿Restaurar la copia de seguridad mÃ¡s reciente?\nArchivo: " + latest_backup + "\nEsto sobrescribirÃ¡ tus perfiles actuales.").c_str(),
                    ("Restore the most recent backup?\nFile: " + latest_backup + "\nThis will overwrite your current profiles.").c_str()
                ));
                dialog->addButton(t("Restaurar", "Restore", "Restaurar"), [latest_backup] {
                    brls::Application::notify(t("Restaurando copia...", "Restoring backup...", "Restaurando cÃ³pia..."));
                    std::string err = pipensx::linkuser::restoreBackup(latest_backup);
                    if (err.empty()) {
                        auto* rebootDialog = new brls::Dialog(t("Â¡Copia restaurada con Ã©xito! Para aplicar los cambios y evitar que la consola crasheÃ©, es obligatorio reiniciar.", "Backup restored successfully! To apply changes and prevent a system crash, you must reboot.", "CÃ³pia restaurada com sucesso! Para aplicar as alteraÃ§Ãµes e evitar que o console trave, Ã© necessÃ¡ria uma reinicializaÃ§Ã£o."));
                        rebootDialog->addButton(t("Reiniciar", "Reboot", "ReinÃ­cio"), [] {
                            pipensx::linkuser::g_shouldReboot = true;
                            brls::Application::quit();
                        });
                        ...
                        auto* errorDialog = new brls::Dialog(t(
                            ("Error al restaurar: " + err + "\n\nNOTA: El acceso a los perfiles requiere permisos del sistema. Debes iniciar esta aplicaciÃ³n desde el Ãlbum (Modo Applet) sin pulsar R para poder restaurar.").c_str(),
...
MainMenu::MainMenu(pipensx::DownloadManager* manager, pipensx::CatalogService* catalog,
    ...
    // Register Actions
    this->registerAction(t("Ajustes", "Settings", "ConfiguraÃ§Ãµes"), brls::BUTTON_BACK, [this](brls::View* view) {
        brls::Application::pushActivity(new brls::Activity(
    ...
    this->registerAction(t("Salir", "Exit", "Sair"), brls::BUTTON_START, [this](brls::View* view) {
        if (manager_ && manager_->hasActiveTransfer()) {
            brls::Dialog* dialog = new brls::Dialog(t(
                "Hay descargas activas o en cola. Si sales ahora, se pausaran.\n\nSeguro que deseas salir?",
                "There are active or queued downloads. Exiting now will pause them.\n\nAre you sure you want to exit?",
                "Existem downloads ativos ou na fila. Sair agora ira pausa-los.\n\nTem certeza de que deseja sair?"));
            dialog->addButton(t("Salir", "Exit", "Sair"), []() {
                brls::Application::quit();
            });
            dialog->addButton(t("Cancelar", "Cancel", "Cancelar"), []() {});
            dialog->open();
    ...
    this->registerAction("Actualizar CFW", brls::BUTTON_Y, [this](brls::View* view) {
        brls::Dialog* dialog = new brls::Dialog(t("El actualizador realiza una instalaciÃ³n limpia. Es recomendable hacer una copia de seguridad de las carpetas que quieras conservar (RetroArch, juegos, homebrew, etc.) antes de actualizar el CFW.\n\nÂ¿Deseas continuar?",
                                                  "The updater performs a clean installation. It is recommended to back up any folders you want to keep (RetroArch, games, homebrew, etc.) before updating the CFW.\n\nDo you want to continue?",
                                                  "O atualizador realiza uma instalaÃ§Ã£o limpa. Recomenda-se fazer backup de todas as pastas que deseja manter (RetroArch, jogos, homebrew, etc.) antes de atualizar o CFW.\n\nDeseja continuar?"));
        ...
        updater_->checkAsync([this](pipensx::UpdateCheckResult result) {
            if (!result.ok) {
                ...
                brls::sync([this, release = result.release]() {
                    std::string esText = "Hay una nueva versiÃ³n de The Goonies APP disponible (" + release.version + "). Â¿Deseas descargarla e instalarla ahora?";
...
#endif
                                    brls::Application::notify(t("ActualizaciÃ³n descargada. Reiniciando...", "Update downloaded. Restarting...", "AtualizaÃ§Ã£o baixada. Reinicializando..."));
                                    brls::Application::quit();
                                });
                            });
                        });
