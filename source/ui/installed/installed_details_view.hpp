...
public:
    InstalledDetailsView(const InstalledTitle& title, GameMetadataService* metadata)
        ...
        brls::Box* actionBox = new brls::Box(brls::Axis::ROW);
        actionBox->setMarginTop(20);
        
        brls::Button* launchBtn = new brls::Button();
        launchBtn->setStyle(&brls::BUTTONSTYLE_BORDERED);
        launchBtn->setText("Lanzar juego");
        launchBtn->setHeight(50);
        launchBtn->setMarginRight(10);
        launchBtn->registerClickAction([title](brls::View*) {
            pipensx::InstalledTitleService::launchTitle(title.applicationId);
            return true;
        });

        brls::Button* deleteGameBtn = new brls::Button();
        deleteGameBtn->setStyle(&brls::BUTTONSTYLE_BORDERED);
        deleteGameBtn->setText("Eliminar juego");
        deleteGameBtn->setHeight(50);
        deleteGameBtn->registerClickAction([this, title](brls::View*) {
            brls::Dialog* dialog = new brls::Dialog("¿Deseas eliminar el juego?");
            dialog->addButton("Si", [this, title]() {
                pipensx::InstalledTitleService::deleteTitle(title.applicationId);
                brls::Application::notify("Eliminado correctamente");
        ...
        actionBox->addView(launchBtn);
        actionBox->addView(deleteGameBtn);
        leftColumn->addView(actionBox);
        ...
        restoreBtn->registerClickAction([title, statusLabel](brls::View*) {
            ...
            dialog->addButton(t("Si", "Yes", "Sim"), [title, statusLabel]() {
                std::string msg;
        ...
        *renderCheats = [cheatsBox, titleId, titleIdStr, renderCheats, cheatsState, listBox]() {
            // Workaround for Borealis focus bug: move focus away from children before deleting them
            if (brls::Application::getCurrentFocus() && brls::Application::getCurrentFocus()->getParent() == cheatsBox) {
            ...
            if (cheatsState->empty()) {
                ...
                downloadBtn->setText("Buscar Trucos Online");
                ...
                noCheats->setText("No se encontraron trucos locales.");
