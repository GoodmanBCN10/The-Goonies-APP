...
namespace GooniesInstaller::libhaze {
namespace {
...
bool Init() {
    ...
    // 1. Install (NSP, NSZ, XCI)
    g_fs_entries.emplace_back(std::make_shared<FsInstallProxy>("1. Install (NSP, NSZ, XCI)", "1. Install (NSP, NSZ, XCI)"));

    // 2. MicroSD
    g_fs_entries.emplace_back(std::make_shared<FsProxy>(std::make_unique<fs::FsNativeSd>(), "2. MicroSD", "2. MicroSD"));
