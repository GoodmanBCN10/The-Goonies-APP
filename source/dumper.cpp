...
#include "backend_usb/usb_uploader.hpp"
#include "backend_usb/usb_dumper.hpp"
#include "backend_usb/usbds.hpp"

namespace GooniesInstaller::dump {
namespace {
...
struct WriteUsbSource final : WriteSource {
    WriteUsbSource(u64 transfer_timeout) {
        ...
        m_usb = std::make_unique<backend_usb::dump::Usb>(transfer_timeout);
    }
...
private:
    std::unique_ptr<backend_usb::dump::Usb> m_usb{};
...
struct UsbTest final : backend_usb::upload::Usb, yati::source::Stream {
    UsbTest(frontend::ProgressBox* pbox, BaseSource* source, std::span<const fs::FsPath> paths, u64 timeout)
    : Usb{timeout}
    , m_pbox{pbox}
    , m_source{source}
    , m_paths{paths} {
    ...
    Result Open(u32 index, s64& out_size, u16& out_flags) override {
        ...
        out_size = size;
        out_flags = backend_usb::api::FLAG_STREAM;
        R_SUCCEED();
    }
...
Result DumpToUsb(frontend::ProgressBox* pbox, BaseSource* source, std::span<const fs::FsPath> paths, const CustomTransfer& custom_transfer) {
...
Result DumpToFile(frontend::ProgressBox* pbox, fs::Fs* fs, const fs::FsPath& root, BaseSource* source, std::span<const fs::FsPath> paths, const CustomTransfer& custom_transfer) {
...
Result DumpToFileNative(frontend::ProgressBox* pbox, BaseSource* source, std::span<const fs::FsPath> paths, const CustomTransfer& custom_transfer) {
    fs::FsNativeSd fs{};
    return DumpToFile(pbox, &fs, "/", source, paths, custom_transfer);
}

Result DumpToStdio(frontend::ProgressBox* pbox, const location::StdioEntry& loc, BaseSource* source, std::span<const fs::FsPath> paths, const CustomTransfer& custom_transfer) {
    fs::FsStdio fs{};
...
Result DumpToUsbS2SInternal(frontend::ProgressBox* pbox, UsbTest* usb) {
...
Result DumpToUsbS2S(frontend::ProgressBox* pbox, BaseSource* source, std::span<const fs::FsPath> paths) {
...
Result DumpToDevNull(frontend::ProgressBox* pbox, BaseSource* source, std::span<const fs::FsPath> paths, const CustomTransfer& custom_transfer) {
    for (auto path : paths) {
        R_TRY(pbox->ShouldExitResult());
...
Result Dump(frontend::ProgressBox* pbox, const std::shared_ptr<BaseSource>& source, const DumpLocation& location, const std::vector<fs::FsPath>& paths, const CustomTransfer& custom_transfer) {
...
void Dump(const std::shared_ptr<BaseSource>& source, const DumpLocation& location, const std::vector<fs::FsPath>& paths, const OnExit& on_exit, const CustomTransfer& custom_transfer) {
    App::Push<frontend::ProgressBox>(0, "Exporting"_i18n, "", [source, paths, location, custom_transfer](auto pbox) -> Result {
