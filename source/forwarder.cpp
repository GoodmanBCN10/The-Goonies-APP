...
std::vector<u8> ReadFileToVector(const std::string& path) {
...
void patch_npdm(std::vector<u8>& npdm, const NpdmPatch& patch) {
    ...
    // SEE: https://github.com/ITotalJustice/GooniesInstaller/issues/67
    u64 ver{};
    splInitialize();
...
void patch_nacp(NacpStruct& nacp, const NcapPatch& patch) {
    ...
    nacp.data_loss_confirmation = 0; // none
    nacp.play_log_policy = 0; // open
    nacp.logo_type = 2; // Nintendo Switch logo
...
auto install_forwader_internal(OwoConfig& config, NcmStorageId storage_id) -> Result {
    ...
    if (config.program_nca.empty()) {
        FileEntries exefs;
        auto main_data = ReadFileToVector("romfs:/forwarder_stub/main");
        auto npdm_data = ReadFileToVector("romfs:/forwarder_stub/main.npdm");
        if (main_data.empty() || npdm_data.empty()) return -1;
        add_file_entry(exefs, "main", main_data);
        add_file_entry(exefs, "main.npdm", npdm_data);

        FileEntries romfs;
        
        std::vector<u8> args_data(config.args.begin(), config.args.end());
        args_data.push_back(0);
        add_file_entry(romfs, "/nextArgv", args_data);
        
        std::vector<u8> nro_path_data(config.nro_path.begin(), config.nro_path.end());
        nro_path_data.push_back(0);
        add_file_entry(romfs, "/nextNroPath", nro_path_data);
...
auto install_forwarder(OwoConfig& config, NcmStorageId storage_id) -> Result {
    return install_forwader_internal(config, storage_id);
}
} // namespace GooniesInstaller
