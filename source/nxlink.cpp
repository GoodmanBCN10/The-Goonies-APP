...
void loop(void* args) {
    ...
    while (!g_quit) {
        ...
        while (!g_quit) {
            ...
            if (R_SUCCEEDED(GooniesInstaller::nro_verify(file_data))) {
                std::string args{};
                ...
                if (R_SUCCEEDED(GooniesInstaller::nro_launch(path, args))) {
                    g_quit = true;
                }
            }
        }
...
bool nxlinkInitialize(NxlinkCallback callback) {
    ...
    Result rc;
    if (R_FAILED(rc = GooniesInstaller::core_utils::CreateThread(&g_thread, loop, nullptr, 1024*64))) {
