...
constexpr const char* kCatalogSourceUrl =
    ...
    "main/switch_games.json";

constexpr const char* kPortsSourceUrl =
    "https://raw.githubusercontent.com/GoodmanBCN10/GooniesPorts-Data/main/catalog.json";
...
size_t headerHttp(char* buffer, size_t size, size_t nitems, void* userdata) {
    std::string* etagOut = static_cast<std::string*>(userdata);
    ...
    if (lowerHeader.find("etag:") == 0) {
        size_t start = 5;
        while (start < header.size() && (header[start] == ' ' || header[start] == '\t'))
            ++start;
        size_t end = header.size();
        while (end > start && (header[end - 1] == '\r' || header[end - 1] == '\n'))
            --end;
        *etagOut = header.substr(start, end - start);
    }
    return total;
}

bool httpGet(const std::string& url, const std::string& etagIn, std::string& etagOut, long& statusOut, std::string& body, std::string& error) {
    body.clear();
    ...
    if (!etagIn.empty()) {
        std::string ifNoneMatch = "If-None-Match: " + etagIn;
        headers = curl_slist_append(headers, ifNoneMatch.c_str());
    }
    ...
    curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, headerHttp);
    ...
    if (result != CURLE_OK || ((statusOut < 200 || statusOut >= 300) && statusOut != 304) ||
        buffer.overflow) {
        if (buffer.overflow)
            ...
            error = "Catalog server returned HTTP " + std::to_string(statusOut) +
                    ".";
        return false;
    }
...
bool CatalogService::isTrustedSource(const std::string& url) {
    ...
    static const char* const kPrefixes[] = {
        "https://raw.githubusercontent.com/Langegen/switch-games/",
        "https://raw.githubusercontent.com/GoodmanBCN10/GooniesPorts-Data/",
    };
...
bool CatalogService::fetchLatest(std::vector<CatalogEntry>& parsed,
    ...
    std::string etagPath = catalogRoot_ + "/catalog.etag";
    std::string cachedEtag;
    std::string ignoredError;
    readFile(etagPath, cachedEtag, ignoredError);
    ...
    if (!httpGet(kCatalogSourceUrl, cachedEtag, newEtag, status, catalogBody, error))
        return false;
        
    if (status == 304) {
        log_msg("[catalog] refresh skipped (304 Not Modified)\n");
    } else {
        catalogModified = true;
    }
        
    std::string portsEtagPath = catalogRoot_ + "/ports.etag";
    std::string cachedPortsEtag;
    readFile(portsEtagPath, cachedPortsEtag, ignoredError);
    
    std::string portsBody;
    std::string newPortsEtag;
    long portsStatus = 0;
    std::string portsError;
    bool portsModified = false;
    
    if (httpGet(kPortsSourceUrl, cachedPortsEtag, newPortsEtag, portsStatus, portsBody, portsError)) {
        if (portsStatus == 304) {
            log_msg("[catalog] ports refresh skipped (304 Not Modified)\n");
        } else {
            portsModified = true;
        }
    }
    
    if (!catalogModified && !portsModified) {
        return true;
    }
    
    // At least one was modified. We need to parse both.
    if (!catalogModified) {
        readFile(cachePath_, catalogBody, ignoredError);
    }
    if (!parseJson(catalogBody, parsed, error))
        return false;
        
    if (!portsModified) {
        readFile(catalogRoot_ + "/ports.json", portsBody, ignoredError);
    }
    if (!portsBody.empty()) {
        nlohmann::json root = nlohmann::json::parse(portsBody, nullptr, false);
        if (!root.is_discarded() && root.is_array()) {
            for (const auto& item : root) {
                if (!item.is_object() || !item.contains("title") || !item.contains("directDownloadUrl")) continue;
                CatalogEntry entry;
                entry.isPort = true;
                entry.title = item["title"].get<std::string>();
                entry.id = item.contains("name") ? item["name"].get<std::string>() : entry.title;
                entry.directDownloadUrl = item["directDownloadUrl"].get<std::string>();
                entry.magnetUri = entry.directDownloadUrl;
                entry.infoHash = entry.id; // Use ID as hash for uniqueness in UI
                if (item.contains("posterUrl")) entry.posterUrl = item["posterUrl"].get<std::string>();
                if (item.contains("category")) entry.category = item["category"].get<std::string>();
                if (item.contains("developer")) entry.developer = item["developer"].get<std::string>();
                if (item.contains("description")) entry.description = item["description"].get<std::string>();
                if (item.contains("publisher")) entry.publisher = item["publisher"].get<std::string>();
                if (item.contains("year")) entry.year = item["year"].get<std::string>();
                if (item.contains("genre")) entry.genre = item["genre"].get<std::string>();
                entry.size = readFlexibleSize(item, "size");
                if (item.contains("screenshots") && item["screenshots"].is_array()) {
                    for (const auto& val : item["screenshots"]) {
                        if (val.is_string() && entry.screenshots.size() < 6) {
                            entry.screenshots.push_back(val.get<std::string>());
                        }
                    }
                }
    ...
    if (portsModified && !portsBody.empty()) {
        writeAtomic(catalogRoot_ + "/ports.json", portsBody, portsError);
        if (!newPortsEtag.empty())
            writeAtomic(portsEtagPath, newPortsEtag, ignoredError);
    }
    
    if (catalogModified && !catalogBody.empty()) {
        if (!writeAtomic(cachePath_, catalogBody, error))
            return false;
        if (!newEtag.empty())
            writeAtomic(etagPath, newEtag, ignoredError);
    }
    
    return true;
}

void CatalogService::loadPortsCache() {
    std::string portsBody;
    std::string error;
    if (readFile(catalogRoot_ + "/ports.json", portsBody, error)) {
        nlohmann::json root = nlohmann::json::parse(portsBody, nullptr, false);
        if (!root.is_discarded() && root.is_array()) {
            for (const auto& item : root) {
                if (!item.is_object() || !item.contains("title") || !item.contains("directDownloadUrl")) continue;
