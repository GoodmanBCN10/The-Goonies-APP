...
auto uiButton::Draw(NVGcontext* vg, Theme* theme) -> void {
    ...
    nvgFillColor(vg, theme->GetColour(ThemeEntryID_TEXT)); // Theme color for the text
    nvgFontSize(vg, 22.f);
    nvgText(vg, m_hint_pos.x, m_hint_pos.y, m_action_str.c_str(), nullptr);
    nvgFillColor(vg, theme->GetColour(ThemeEntryID_TEXT)); // White for the button glyph
    nvgFontSize(vg, 28.f);
