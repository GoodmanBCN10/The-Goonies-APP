...
namespace GooniesInstaller::frontend::menu::appstore {
namespace {

constexpr fs::FsPath REPO_PATH{"/switch/GooniesInstaller/cache/appstore/repo.json"};
constexpr fs::FsPath CACHE_PATH{"/switch/GooniesInstaller/cache/appstore"};
...
auto InstallApp(ProgressBox* pbox, const Entry& entry) -> Result {
    static const fs::FsPath zip_out{"/switch/GooniesInstaller/cache/appstore/temp.zip"};
...
void EntryMenu::Draw(NVGcontext* vg, Theme* theme) {
    ...
    text_start_y += text_inc_y;
    gfx::drawTextArgs(vg, text_start_x, text_start_y, font_size, NVG_ALIGN_LEFT | NVG_ALIGN_TOP, theme->GetColour(ThemeEntryID_TEXT), "extracted: %s"_i18n.c_str(), core_utils::formatSizeStorage(m_entry.extracted).c_str());
    text_start_y += text_inc_y;
...
Menu::Menu(u32 flags) : grid::Menu{"AppStore"_i18n, flags} {
    fs::FsNativeSd fs;
    fs.CreateDirectoryRecursively("/switch/GooniesInstaller/cache/appstore/icons");
    fs.CreateDirectoryRecursively("/switch/GooniesInstaller/cache/appstore/banners");
    fs.CreateDirectoryRecursively("/switch/GooniesInstaller/cache/appstore/screens");
        ...
        if (e.binary.empty() || e.binary == "none") {
            ...
            if (fs.FileExists(e.binary)) {
                ...
                if (e.status == EntryStatus::Get) {
                    ...
                    // ignore hbmenu if it was replaced with GooniesInstaller.
                    if (e.name == "hbmenu") {
                        NacpStruct nacp;
