#include "frontend/menus/game_nca_menu.hpp"
...
namespace GooniesInstaller::frontend::menu::game::meta_nca {
namespace {
...
struct NcaSource final : dump::BaseSource {
    ...
    Result Read(const std::string& path, void* buf, s64 off, s64 size, u64* bytes_read) override {
        const auto it = std::ranges::find_if(m_entries, [&path](auto& e){
            return path.find(core_utils::hexIdToStr(e.content_id).str) != path.npos;
        });
    ...
    auto GetName(const std::string& path) const -> std::string {
        ...
        if (it != m_entries.end()) {
            return core_utils::hexIdToStr(it->content_id).str;
        }
        ...
        std::make_pair(Button::X, Action{"Options"_i18n, [this](){
            ...
            if (!m_entries.empty()) {
                ...
                options->Add<SidebarEntryCallback>("Verify NCA 256 hash"_i18n, [this](){
                    ...
                    App::Push<ProgressBox>(m_entry.image, "Hashing"_i18n, core_utils::hexIdToStr(GetEntry().content_id).str, [this](auto pbox) -> Result{
                        ...
                        const auto str = core_utils::hexIdToStr(GetEntry().content_id);
...
void Menu::Draw(NVGcontext* vg, Theme* theme) {
    ...
    m_list->Draw(vg, theme, m_entries.size(), [this](auto* vg, auto* theme, auto& v, auto i) {
        ...
        gfx::drawTextArgs(vg, x + text_xoffset + 150, y + (h / 2.f), 20.f, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE, theme->GetColour(text_id), "%s", core_utils::hexIdToStr(e.content_id).str);
        gfx::drawTextArgs(vg, x + w - text_xoffset, y + (h / 2.f), 16.f, NVG_ALIGN_RIGHT | NVG_ALIGN_MIDDLE, theme->GetColour(ThemeEntryID_TEXT_INFO), "%s", core_utils::formatSizeStorage(e.size).c_str());
...
Result Menu::MountNcaFs() {
    ...
    if (R_FAILED(GetFsFileSystemType(e.header.content_type, type))) {
        ...
        filebrowser::MountFsHelper(fs, core_utils::hexIdToStr(e.content_id).str);
    } else {
