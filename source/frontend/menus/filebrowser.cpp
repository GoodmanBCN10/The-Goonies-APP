...
#include "frontend/menus/file_viewer.hpp"
#include "frontend/menus/image_viewer.hpp"
...
#include "frontend/music_player.hpp"
...
namespace GooniesInstaller::frontend::menu::filebrowser {
namespace {
...
, m_arg_path{arg_path} {
    ...
    auto callback = this->Add<SidebarEntryCallback>("Create", [this, file_name](){
        ...
        if (!m_db_indexs.empty()) {
            fs::FsNativeSd().read_entire_file("/config/GooniesInstaller/logo/rom/NintendoLogo.png", config.logo);
            fs::FsNativeSd().read_entire_file("/config/GooniesInstaller/logo/rom/StartupMovie.gif", config.gif);
        }
...
void FsView::Draw(NVGcontext* vg, Theme* theme) {
    ...
    m_list->Draw(vg, theme, m_entries_current.size(), [this, text_col, &got_dir_count](auto* vg, auto* theme, auto& v, auto i) {
        ...
        NVGcolor card_color = nvgRGBA(255, 255, 255, 15);
        if (selected) {
            card_color = nvgRGBA(255, 255, 255, 40);
        }
        nvgBeginPath(vg);
        nvgRoundedRect(vg, x + 10.f, y + 4.f, w - 20.f, h - 8.f, 10.f);
        nvgFillColor(vg, card_color);
        nvgFill(vg);
        ...
        if (e.IsDir() && !m_fs_entry.IsNoStatDir() && (e.dir_count != -1 || !e.done_stat)) {
            ...
            gfx::drawTextArgs(vg, x + w - text_xoffset, y + (h / 2.f) - 3, 16.f, NVG_ALIGN_RIGHT | NVG_ALIGN_BOTTOM, theme->GetColour(ThemeEntryID_TEXT_INFO), "%s", core_utils::formatSizeStorage(e.file_size).c_str());
        }
    });
}
...
void FsView::InstallFiles() {
    ...
    App::Push<OptionBox>("Install Selected files?"_i18n, "No"_i18n, "Yes"_i18n, 0, [this, targets](auto op_index){
        if (op_index && *op_index) {
            ...
            App::Push<frontend::ProgressBox>(0, "Installing "_i18n, "", [this, targets](auto pbox) -> Result {
                for (auto& e : targets) {
...
void FsView::UnzipFiles(fs::FsPath dir_path) {
    ...
    App::Push<frontend::ProgressBox>(0, "Extracting "_i18n, "", [this, dir_path, targets](auto pbox) -> Result {
...
void FsView::ZipFiles(fs::FsPath zip_out) {
    ...
    App::Push<frontend::ProgressBox>(0, "Compressing "_i18n, "", [this, zip_out, targets](auto pbox) -> Result {
...
void Base::LoadAssocEntries() {
    if (m_options & FsOption_LoadAssoc) {
        ...
        LoadAssocEntriesPath("/config/GooniesInstaller/assoc/");
    }
}
