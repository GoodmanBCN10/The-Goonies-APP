#include "app.hpp"
#include "frontend/menus/grid_menu_base.hpp"
...
Vec4 Menu::DrawEntry(NVGcontext* vg, Theme* theme, bool draw_image, int layout, const Vec4& v, bool selected, int image, const char* name, const char* author, const char* version) {
    ...
    NVGcolor card_color = nvgRGBA(255, 255, 255, 15); // Glossy translucent white
    if (selected) {
        card_color = nvgRGBA(255, 255, 255, 40); // Brighter when selected
    }

    nvgBeginPath(vg);
    nvgRoundedRect(vg, x, y, w, h, 12.f);
    nvgFillColor(vg, card_color);
    nvgFill(vg);

    if (selected) {
        nvgStrokeWidth(vg, 3.f);
        nvgStrokeColor(vg, theme->GetColour(ThemeEntryID_TEXT_SELECTED)); // Use theme's accent color
        nvgStroke(vg);
    }
    ...
    } else if (layout == LayoutType_Grid) {
        // padding for grid items to float inside the card
        image_v.x += 10;
        image_v.y += 10;
        image_v.w -= 20;
        image_v.h -= 40;
        
        // Draw the text inside the card, just below the image
        m_scroll_name.Draw(vg, selected, x + 10, image_v.y + image_v.h + 5, w - 20, 18.f, NVG_ALIGN_LEFT | NVG_ALIGN_TOP, theme->GetColour(text_id), name);
    } else {
        if (selected) {
    ...
    if (draw_image) {
        if (image > 0) {
            gfx::drawImage(vg, image_v, image, 8);
        } else {
