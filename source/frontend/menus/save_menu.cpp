...
namespace GooniesInstaller::frontend::menu::save {
namespace {

constexpr u32 NX_SAVE_META_MAGIC = 0x4A4B5356;
...
// Metadata implementation details reference issue 264
struct NXSaveMeta {
...
void Menu::DisplayOptions() {
    auto options = std::make_unique<Sidebar>("Opciones de Partida"_i18n, Sidebar::Side::LEFT);
    ...
    options->Add<SidebarEntryCallback>("Sort By"_i18n, [this](){
        auto options = std::make_unique<Sidebar>("Sort Options"_i18n, Sidebar::Side::LEFT);
        ON_SCOPE_EXIT(App::Push(std::move(options)));
    ...
    if (m_entries.size()) {
        if (!m_selected_count || m_selected_count == 1) {
            options->Add<SidebarEntryCallback>("Extraer a..."_i18n, [this](){
                ...
                "Extrae tus partidas a la microSD o el PC"_i18n
            );
        }

        if (m_entries[m_index].save_data_type == FsSaveDataType_Account || m_entries[m_index].save_data_type == FsSaveDataType_Bcat) {
            options->Add<SidebarEntryCallback>("Importar"_i18n, [this](){
                RestoreSave();
            }, true,
                "Importa tus partidas guardadas desde la microSD o desde el PC"_i18n
            );
        }

        options->Add<SidebarEntryCallback>("Eliminar partida guardada"_i18n, [this](){
            App::Push<OptionBox>(
                "¿Estás seguro de que quieres eliminar esta partida guardada?\n¡Esta acción no se puede deshacer!"_i18n,
                "Cancelar"_i18n, "Eliminar"_i18n, 0, [this](auto op_index){
                    if (op_index && *op_index) {
                        App::Push<ProgressBox>(0, "Eliminando..."_i18n, "", [this](auto pbox) -> Result {
                            return fsDeleteSaveDataFileSystemBySaveDataSpaceId(
                                (FsSaveDataSpaceId)m_entries[m_index].save_data_space_id,
                                m_entries[m_index].save_data_id
                            );
                        }, [this](Result rc){
                            App::PushErrorBox(rc, "Delete failed!"_i18n);
                            if (R_SUCCEEDED(rc)) {
                                App::Push<OptionBox>("Partida eliminada."_i18n, "OK"_i18n);
                                m_dirty = true;
                            }
                        });
                    }
        ...
        }, true, "Eliminar permanentemente la partida seleccionada del sistema."_i18n);
    }
...
void Menu::BackupSaves(std::vector<std::reference_wrapper<Entry>>& entries, u32 flags) {
    dump::DumpGetLocation("Select backup location"_i18n, dump::DumpLocationFlag_SdCard|dump::DumpLocationFlag_Stdio|dump::DumpLocationFlag_Usb, [this, entries, flags](const dump::DumpLocation& location){
        App::Push<ProgressBox>(0, "Backup"_i18n, "", [this, entries, location, flags](auto pbox) -> Result {
            ...
            if (R_SUCCEEDED(rc)) {
                App::Push<OptionBox>("Guardado completado"_i18n, "OK"_i18n);
            }
        });
    });
}
...
void Menu::RestoreSave() {
    dump::DumpGetLocation("Selecciona la ubicacion de la partida guardada"_i18n, dump::DumpLocationFlag_SdCard|dump::DumpLocationFlag_Stdio, [this](const dump::DumpLocation& location){
        ...
        App::Push<PopupList>(
            title, items, [this, paths, items, location](auto op_index){
                ...
                App::Push<OptionBox>(
                    ...
                    "Atrás"_i18n, "Importar"_i18n, 0, [this, file_path, location](auto op_index){
                        if (op_index && *op_index) {
                            App::Push<ProgressBox>(0, "Restore"_i18n, "", [this, file_path, location](auto pbox) -> Result {
                                ...
                                if (R_SUCCEEDED(rc)) {
                                    App::Push<OptionBox>("¡Partida importada!"_i18n, "OK"_i18n);
                                }
                            });
                        }
...
Result Menu::BackupSaveInternal(ProgressBox* pbox, const dump::DumpLocation& location, std::span<const std::reference_wrapper<Entry>> entries, u32 flags) {
    ...
    return dump::Dump(pbox, source, location, paths, [&](frontend::ProgressBox* pbox, dump::BaseSource* _source, dump::WriteSource* writer, const fs::FsPath& path) -> Result {
...

        {
            ...
            ON_SCOPE_EXIT(zipClose(zfile, "GooniesInstaller v" APP_DISPLAY_VERSION));
