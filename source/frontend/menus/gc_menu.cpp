...
namespace GooniesInstaller::frontend::menu::gc {
namespace {
...
Result NszExport(ProgressBox* pbox, const keys::Keys& keys, dump::BaseSource* _source, dump::WriteSource* writer, const fs::FsPath& path) {
    ...
    for (u32 i = 0; i < std::size(root.partitions); i++) {
        ...
        const auto nca_creator = [&yati_source](const nca::Header& header, const keys::KeyEntry& title_key, const core_utils::nsz::Collection& collection) {
        ...
        std::make_pair(Button::A, Action{"OK"_i18n, [this](){
            ...
            if (m_option_index == 0) {
                if (!App::GetInstallEnable()) {
                    ...
                    App::Push<frontend::ProgressBox>(m_icon, "Installing "_i18n, m_entries[m_entry_index].lang_entry.name, [this](auto pbox) -> Result {
