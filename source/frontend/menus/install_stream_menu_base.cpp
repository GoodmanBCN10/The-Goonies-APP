...
namespace GooniesInstaller::frontend::menu::stream {
namespace {
...
void Menu::Update(Controller* controller, TouchInfo* touch) {
    ...
    if (m_state == State::Connected) {
        ...
        std::string filename = m_source->GetPath();
        App::Push<frontend::ProgressBox>(0, "Conectado al PC", filename, [this, filename](auto pbox) -> Result {
            std::string basename = filename;
            auto pos = basename.find_last_of('/');
            if (pos != std::string::npos) basename = basename.substr(pos + 1);
            pbox->NewTransfer(basename);
            ...
            m_last_install_size = std::max(pbox->GetSize(), pbox->GetOffset());
            ...
            m_history.push_back({filename, R_SUCCEEDED(rc), m_last_install_size.load()});

            if (R_SUCCEEDED(rc)) {
                // App::Notify("Install success!"_i18n); // Removed per user request
                m_state = State::Done;
            } else {
