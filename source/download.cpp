...
namespace GooniesInstaller::curl {
namespace {
...
private:
    ...
    static constexpr inline fs::FsPath JSON_PATH{"/switch/GooniesInstaller/cache/etag_v2.json"};
...
struct ThreadEntry {
    auto Create() -> Result {
        ...
        R_TRY(core_utils::CreateThread(&m_thread, ThreadFunc, this, 1024*32));
...
void GetDownloadTempPath(fs::FsPath& buf) {
    ...
    std::snprintf(buf, sizeof(buf), "/switch/GooniesInstaller/cache/download_temp%lu", count_copy);
}
