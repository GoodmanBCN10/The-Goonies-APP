...
using CustomTransfer = std::function<Result(frontend::ProgressBox* pbox, BaseSource* source, WriteSource* writer, const fs::FsPath& path)>;
...
Result Dump(frontend::ProgressBox* pbox, const std::shared_ptr<BaseSource>& source, const DumpLocation& location, const std::vector<fs::FsPath>& paths, const CustomTransfer& custom_transfer = nullptr);
